...
export default function PrijavaPage() {
  useEffect(() => {
    ...
    // Učitaj Google Identity Services SDK
  ...
  const handleGoogleLogin = () => {
    if (!window.google || !window.google.accounts) {
      setModal({ open: true, message: "Google login nije dostupan. Pokušajte kasnije." });
      return;
    }
    window.google.accounts.id.initialize({
      ...
      callback: async (response) => {
        if (!response.credential) {
          setModal({ open: true, message: "Google prijava nije uspjela." });
          return;
        }
        setIsLoading(true);
        try {
          ...
          if (!res.ok) {
            const data = await res.json().catch(() => ({}));
            setModal({ open: true, message: data.detail || "Google prijava nije uspjela." });
            setIsLoading(false);
            return;
          }
          const data = await res.json();
        ...
        <div className={styles.container}>
          <div className={styles.formContainer}>
            ...
            <div className={styles.authCard}>
              ...
              <button onClick={handleGoogleLogin} className={styles.googleButton} ref={googleBtnRef} type="button">

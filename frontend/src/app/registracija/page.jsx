...
export default function RegistracijaPage() {
  ...
  const GOOGLE_CLIENT_ID = "667648269284-qdkle11dbr000casqko3b7otr66pogsb.apps.googleusercontent.com";
  ...
  const handleGoogleSignup = () => {
    if (!window.google || !window.google.accounts) {
      setModal({ open: true, message: "Google registracija nije dostupna. Pokušajte kasnije." });
      return;
    }
    window.google.accounts.id.initialize({
      ...
      callback: async (response) => {
        if (!response.credential) {
          setModal({ open: true, message: "Google registracija nije uspjela." });
          return;
        }
        setIsLoading(true);
        try {
          const res = await fetch("http://localhost:8000/auth/google-register", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          ...
          if (!res.ok) {
            ...
            setModal({ open: true, message: data.detail || "Google registracija nije uspjela." });
            setIsLoading(false);
            return;
          }
        ...
        <div className={styles.container}>
          <div className={styles.formContainer}>
            ...
            <div className={styles.authCard}>
              ...
              <button onClick={handleGoogleSignup} className={styles.googleButton} ref={googleBtnRef} type="button">
